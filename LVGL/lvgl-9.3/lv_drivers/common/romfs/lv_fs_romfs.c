...
/*API for RomFS. */
#define LV_USE_FS_ROMFS 1
#if LV_USE_FS_ROMFS
    #define LV_USE_FS_ROMFS_LETTER 'A'     /*Set an upper cased letter on which the drive will accessible (e.g. 'A')*/
#endif
 ...
 *  STATIC PROTOTYPES
 **********************/
static lv_fs_drv_t romfs_fs_drv;
 ...
 */
void lv_fs_romfs_init(void)
{
    /*---------------------------------------------------
     * Register the file system interface in LVGL
     *--------------------------------------------------*/

    lv_fs_drv_t * fs_drv_p = &romfs_fs_drv;
    lv_fs_drv_init(fs_drv_p);

    romfs_mount((void *)ROM_FS_ADDR);

    /*Set up fields...*/
    fs_drv_p->letter = LV_USE_FS_ROMFS_LETTER;
...
static lv_fs_res_t fs_write(lv_fs_drv_t * drv, void * file_p, const void * buf, uint32_t btw, uint32_t * bw)
{
    ...
    int fd = FILEP2FD(file_p);
    ssize_t ret = r_write(fd, buf, btw);
    if(ret < 0) {
...
static lv_fs_res_t fs_tell(lv_fs_drv_t * drv, void * file_p, uint32_t * pos_p)
{
    ...
    int fd = FILEP2FD(file_p);
    off_t offset = r_lseek(fd, 0, SEEK_CUR);
    if(offset < 0) {
